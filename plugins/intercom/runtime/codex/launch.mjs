...
const bridgePath = process.env.INTERCOM_BRIDGE_PATH ||
  fileURLToPath(new URL('../bridge/bridge.mjs', import.meta.url))
appServerArgs.push(
  '-c', `mcp_servers.intercom.command=${JSON.stringify('node')}`,
  '-c', `mcp_servers.intercom.args=${JSON.stringify([bridgePath])}`
)
